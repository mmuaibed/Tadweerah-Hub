...
- **License Gates:** POST /offers: buyers lacking a required capability receive 403 MissingCapability; capabilities with `requires_license = true` where the buyer lacks `license_status = approved` receive 403 LicenseRequired. POST /listings: if `material_category_id` has `is_sensitive=true`, the producer must also have `license_status = approved` (403 LicenseRequired) before creating the listing.
