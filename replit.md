...
- **MWAN-Ready Architecture:** Multi-role companies via `company_roles` junction table (producer/buyer/carrier). Role enum maps to MWAN terminology: generator/receiver/transporter. Legacy `type` column preserved for backward compat. `transport_requests` table has full MWAN-aligned status machine: `pending → accepted → manifest_ready → in_transit → delivered → closed | cancelled`. `manifest_records` stub table added. Onboarding includes MWAN role checkbox group. Dashboard shows carrier-specific transport section when company has carrier role. New `/transport-requests` page with mine/available tabs and carrier accept action. MWAN eManifest readiness panel (collapsible) in deal-panel for payment_confirmed+ deals — fetches from `GET /api/deals/:id/mwan-summary` and shows score, party data, and 14-point checklist.
