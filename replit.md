...
- **Database Schema:** Includes tables for `companies` (with `license_status`, `license_number`, `license_document_url`, `company_category_id`, `accepted_terms_at`, nullable `type`), `waste_listings` (with `sale_type`, `unit_option_id`, `material_category_id`, `revenue_share_pct`), `deals` (with `payment_reference`, `payment_proof_url`), plus lookup tables `capabilities`, `company_capabilities`, `company_categories`, `unit_options`, `material_categories`, `company_actions` (8 seeds: sell, buy, transport, recycle, safe_disposal, sorting, storage, other), `company_action_selections` (join table: company → chosen actions).
