...
- **Pages:** /terms (Terms & Conditions static page), /reports (real stats from GET /dashboard/stats + compliance badge + subscription-gated future metrics).
- **Value Layer (V1–V7):** `DealPanel` enhanced with: (V1) DealValueSummary showing auction competition financial gain (producer-only, non-revenue-share); (V2) vertical GovernanceTimeline with 4 steps and pending/done state; (V3) printDealReport opens a new print window with full bilingual HTML report; (V5) smart contextual messages; (V6) green compliance/trust badge for completed deals. Dashboard (V4) shows a live stats strip from `GET /dashboard/stats` (listings, offers received/made, completed deals, deal value). Reports page (V7) shows real operational impact summary from the same stats endpoint. `GET /dashboard/stats` endpoint returns aggregated company metrics.
- **listing-detail.tsx:** Passes `listingRef`, `listingMaterial`, `listingCategory`, `listingQuantity`, `myCompanyName` to DealPanel for print reports; `listing_id` is included in DealInfo and deal serializer. Material category name shown as a DetailRow (label: "الفئة / Category") derived from API JOIN.
- **Deal reference:** `dealRef(id, createdAt)` in `artifacts/tadweerah/src/lib/listing-ref.ts` derives `TDW-{YEAR}-{XXXXXX}` from UUID + creation year. Shown in DealPanel header (monospaced) and print report. No schema change.
- **Material category name:** All listing queries (GET /listings, GET /listings/mine, GET /listings/:id, close re-fetch) now LEFT JOIN `materialCategoriesTable` and return `material_category_name_ar`/`material_category_name_en`. Shown in: listing cards (green pill badge), listing detail (DetailRow), deal panel print report (new category row).
- **Pre-launch polish batch (implemented):**
  - **Contact card (prominent):** DealPanel contact section replaced with a green "تهانينا" card (CheckCircle2 icon, congratulations message, company name, large clickable `tel:` link). Falls back gracefully if phone is empty. Print report now includes both parties' phone numbers (producerPhone/buyerPhone derived from `role` + `myPhone` prop vs `deal.counterparty.contact_phone`).
  - **TDW reference:** Now displayed as `font-bold font-mono tracking-wider text-sm` (was `text-[10px] text-primary/50`). Inline copy button (Copy → Check icon on success, "تم النسخ" feedback label). Deal subtitle shows "material · counterpartyName" below "تفاصيل الصفقة". `myPhone` prop added to DealPanel and passed from listing-detail via `me.company.contactPhone`.
  - **Print report redesign:** Full HTML overhaul — Tadweerah gradient "ت" logo icon, `سجل العملية التجارية / Transaction Record` title, deal ref prominently in header, `doc-note` banner with creation timestamp, green `total-box` with large total value, contact section with both parties' phones (monospace styled), payment_reference row if set, professional section headers with blue underlines, footer "وُثِّقت عبر المنصة / Documented via platform".
  - **Unit "other" free text:** `waste_listings` gained nullable `unit_notes text` column (direct ALTER TABLE + Drizzle schema + lib/db rebuild). API baseSelect/serialize/POST all handle `unit_notes`. Listing form: `isOtherUnit` derived from selected unit key; conditional `<Input>` appears below unit select when key="other"; value sent as `unit_notes` in create body.
  - **Deal creation date:** Shown as last row in settlement summary grid (formatted locale date).
  - **Deal title clarity:** Subtitle row below "تفاصيل الصفقة" shows `listingMaterial · counterpartyName` (truncated).
