...
- **License Gates:** Eligibility checks apply only to offer participation, never to listing creation. POST /offers: buyers lacking a required capability receive 403 MissingCapability; capabilities with `requires_license = true` where the buyer lacks `license_status = approved` receive 403 LicenseRequired. Any company may create listings regardless of license status.
...
- **Database Schema:** Includes tables for `companies` (with `license_status`, `license_number`, `license_document_url`, `company_category_id`, `accepted_terms_at`, nullable `type`), `waste_listings` (with `sale_type`, `unit_option_id`, `material_category_id`, `revenue_share_pct`, `targeting_type` enum [open/category/specific_company], `target_company_id`), `deals` (with `payment_reference`, `payment_proof_url`), plus lookup tables `capabilities`, `company_capabilities`, `company_categories`, `unit_options`, `material_categories` (with `is_sensitive` boolean), `company_actions` (8 seeds: sell, buy, transport, recycle, safe_disposal, sorting, storage, other), `company_action_selections` (join table: company → chosen actions), `listing_target_categories` (join: listing → allowed company categories for category-targeting).
